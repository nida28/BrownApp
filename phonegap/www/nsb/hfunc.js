// //Copyright 2016 NS BASIC Corporation
...
return;}
if(typeof(NSB.Printobj)!=='undefined'&&NSB.Printobj!==null){if(!s)return;if(NSB_text.innerHTML.slice(-4)!=="<br>")s="<br>"+s;NSB_text.innerHTML=NSB_text.innerHTML+s;NSB.Printobj.style.display="block";if(typeof(IScroll)!=="undefined"){setTimeout(function(){NSB.Printref.refresh();},100);}}else{var height=window.innerHeight-50;NSB.Printobj=document.createElement('div');NSB.Printobj.style.position='fixed';NSB.Printobj.style.top='25px';NSB.Printobj.style.left='10%';NSB.Printobj.style.width='80%';NSB.Printobj.style.height=height+'px';NSB.Printobj.style.zIndex=1;NSB.Printobj.id='NSB_Progress';NSB.Printobj.innerHTML="<div id='NSB_scroller' style='height:"+(height-5)+"px;'>"+"<div id='NSB_text' style='padding:10px; font-family:helvetica; font-size:12px;'>"+s+"</div>"+"<div id='NSB_ProgressClose' class='NSB_ProgressClose'>&times;</div>";document.body.appendChild(NSB.Printobj);NSB_Progress.onclick=function(){NSB_Progress.style.display='none';};if(typeof(IScroll)!=="undefined"){NSB.Printref=new IScroll(NSB_scroller,{bounce:true,zoom:false,mouseWheel:true,scrollbars:true});setTimeout(function(){NSB.Printref.refresh();},200);}}};NSB.ShowProgress=function(s){if(s===false||!s){if(typeof(NSB.ShowProgressobj)==='object'&&NSB.ShowProgressobj!==null){if(NSB.ShowProgressobj.parentNode!==undefined){$.jqmodal.close();NSB.ShowProgressobj.parentNode.removeChild(NSB.ShowProgressobj);NSB.ShowProgressobj=null;}}
return;}
