/**************************************************************************************/
// Bootstrap functions
// copyright 2016 NS BASIC Corporation. All rights reserved.
...
NSB.bs.addProperties = function(ctrl, type){
    ...
    ctrl.hide = function(options) {
        //skanky way to determine if called by jQuery, not user.
        if (ctrl.hide.caller && ctrl.hide.caller.toString().startsWith("function (b,c,e,f)")) return;
...
NSB.bs.tabs = {
    ...
    make: function (id, disabled, items) {
        var i, s, k, arrItems, item, onclick, idprop;
        arrItems = items.split("\n");
        if (items === '') arrItems = [];
